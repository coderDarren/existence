        ...
        public override void OnInspectorGUI() {
            ...
            if ( BeginCenteredGroup( "GENERAL SETTINGS", ref reflections.folds[0] ) ) {
                ...
                if ( reflections.Settings.useDepth ) {
                    reflections.internalDepthShader = ObjectField<Shader>( new GUIContent( "DEPTH/ADDITIONAL PASS SHADER", "The shader used to render the depth/custom pass of the reflections" ), reflections.internalDepthShader );
                    GUILayout.Space( 8 );
                }
            ...
            if ( BeginCenteredGroup( "REFLECTION SETTINGS", ref reflections.folds[1] ) ) {
                ...
                if ( !reflections.isSRP ) {
                    GUILayout.Space( 8 );
                    reflections.Settings.customShadowDistance = EnableDisableToggle( new GUIContent( "CUSTOM SHADOWS DISTANCE", "If enabled, the shadows rendering distance defined on the Quality Settings will be overriden by this reflection renderer" ), reflections.Settings.customShadowDistance );
            ...
            if ( BeginCenteredGroup( "OPTIMIZATION SETTINGS", ref reflections.folds[2] ) ) {
                ...
                if ( !reflections.ExternalReflectionTex ) {
                    ...
                    if ( !reflections.ExternalReflectionTex ) {
                        ...
                        reflections.Settings.resolutionDownscale = SliderField( new GUIContent( "REFLECTION DOWNSCALE", "The final value that the resolution of this reflection will be multiplied by." ), reflections.Settings.resolutionDownscale, 0.05f, 2.0f );
                    }
            ...
            if ( reflections.Settings.useCustomComponents ) {
                if ( BeginCenteredGroup( "CUSTOM COMPONENT SETTINGS", ref reflections.folds[4] ) ) {
                    ...
                    reflections.Settings.autoSynchComponents = EnableDisableToggle( new GUIContent( "AUTO-SYNCH COMPONENTS", "Automatically synch the values for all the fields of the components attached to this reflection renderer" ), reflections.Settings.autoSynchComponents );
